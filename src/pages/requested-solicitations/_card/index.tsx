...
export default function PageCard({ placeReservation, shouldClick = true }: PageCardProps) {
  ...
  const getFormattedDate = (date: string) => {
    return moment.tz(date, "YYYY-MM-DDT00:00:00.000Z", "America/Sao_Paulo")
      .format("DD/MM/YYYY 00:00").toString();
  }

  const updateReservationStatus = (accepted: boolean) => {
    updatePlaceReservation(placeReservation.id, accepted ? "ACCEPTED" : "REJECTED").then((response) => {
    ...
    <>
      <Card hasMargin={false} aditionalStyles={`${styles.cardStyle} ${!shouldClick && styles.noClick}`} onClick={openModal}>
        ...
        <section className={styles.cardInfoCont}>
          <strong className={styles.cardTitle}>{placeReservation.product?.title}</strong>
          <strong className={styles.cardInfo}>Início:{" "}
            <span className={styles.cardInfoValue}>{getFormattedDate(placeReservation.startsAt)}</span>
          </strong>
          <strong className={styles.cardInfo}>Fim:{" "}
            <span className={styles.cardInfoValue}>{getFormattedDate(placeReservation.endsAt)}</span>
          </strong>
          <strong className={styles.cardInfo}>Qnt. pessoas:{" "}
            <span className={styles.cardInfoValue}>{placeReservation.numOfParticipants}</span>
          </strong>
          <strong className={styles.cardInfo}>Valor:{" "}
            <span className={styles.cardInfoValue}>R$ {placeReservation.placeFinalPrice}</span>
          </strong>
        ...
        <section className={`${styles.cardInfoCont} ${styles.userInfoCont}`}>
          ...
          <UserAccountCircle 
            image={placeReservation.eventOwner.profilePhoto == 'string' ? null : placeReservation.eventOwner.profilePhoto || null} 
          />
          <strong className={styles.userName}>{placeReservation.eventOwner.name}</strong>
          <span className={styles.userInfo}>{placeReservation.eventOwner.email}</span>
          <span className={styles.userInfo}>{placeReservation.eventOwner.telephone}</span>
        </section>
      </Card>
      ...
      {isModalOpen &&
        <Modal onClose={() => setIsModalOpen(false)}>
          ...
          <section className={styles.buttonsContainer}>
            <button className={styles.button} onClick={() => updateReservationStatus(true)}>
              <FiThumbsUp color="#FFF" size={24} />
              <span className={styles.buttonText}>Aceitar</span>
            </button>
            <button className={`${styles.button} ${styles.secButton}`} onClick={() => updateReservationStatus(false)}>
