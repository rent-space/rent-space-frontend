...
import { getPlaceReservationById, getPlaceReservations } from "@/services/api/reservations";
...
export default function RequestedSolicitations() {
  ...
  const [allReservations, setAllReservations] = useState<PlaceReservation[] | null>(null);
  const [pedingReservation, setPendingReservation] = useState<PlaceReservation[] | null>(null);
  const [answeredReservation, setAnsweredReservation] = useState<PlaceReservation[] | null>(null);
  ...
  useEffect(() => {
    if (data?.user?.userType === 'PLACE_OWNER' || data?.user?.userType === 'SERVICE_OFFER') {
      const pageInfos = {
  ...
  const getReservationsForUser = (userEmail: string) => {
    if (allReservations != null) {
      ...
      allReservations.forEach(reserv => {
        ...
        } else if ((reserv.product.owner.email == userEmail || 
          reserv.eventOwner.email == userEmail)
        ) {
            ...
            {isLoading ?
              ...
              pedingReservation !== null && pedingReservation.length > 0 ?
                pedingReservation.map((reser, index) =>
                  <PageCard placeReservation={reser} key={index} shouldClick={shouldRespond} />
                ) :
        ...
        <div className={styles.cardsContainer}>
          ...
          <div className={styles.cards}>
            {isLoading ?
              <Loading loadingLabel="Carregando suas reservas..." />
              :
              answeredReservation !== null && answeredReservation.length > 0 ?
                answeredReservation.map((reser, index) =>
                  <PageCard placeReservation={reser} key={index} shouldClick={false} />
                ) :
              <span>Não há nenhuma solicitação</span>
            }
          </div>
